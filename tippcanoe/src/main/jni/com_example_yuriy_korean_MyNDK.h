/* DO NOT EDIT THIS FILE - it is machine generated */
#include <jni.h>
/* Header for class com_example_yuriy_korean_MyNDK */

#ifndef _Included_com_example_yuriy_korean_MyNDK
#define _Included_com_example_yuriy_korean_MyNDK
#ifdef __cplusplus
extern "C" {
#endif
/*
 * Class:     com_example_yuriy_korean_MyNDK
 * Method:    getMyString
 * Signature: ()Ljava/lang/String;
 */
JNIEXPORT jstring JNICALL Java_com_example_yuriy_korean_MyNDK_getMyString
//  (JNIEnv *, jobject, jstring, jstring, jstring,jint,jobjectArray);
  (JNIEnv *, jobject, jstring, jstring, jstring);


JNIEXPORT jstring JNICALL Java_com_example_yuriy_korean_MyNDK_startTippecanoe
//  (JNIEnv *, jobject, jstring, jstring, jstring,jint,jobjectArray);
        (JNIEnv *, jobject, jstring, jstring,jint ,jobjectArray);

#ifdef __cplusplus
}
#endif
#endif
