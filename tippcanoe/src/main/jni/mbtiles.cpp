...
void check_after_open() {
	__android_log_assert("cond", "runMain", "inside exit(EXIT_FAILURE) for sqlite3_open");
}

void check_inside_open() {
	__android_log_assert("cond", "runMain", "inside exit(EXIT_FAILURE) for sqlite3_open");
}

sqlite3 *mbtiles_open(char *dbname, char **argv, int forcetable) {
	sqlite3 *outdb;
//	__android_log_assert("cond", "runMain", "inside mbtiles_open");
	__android_log_write(ANDROID_LOG_ERROR, "mbtiles_open", dbname);
	if (sqlite3_open(dbname, &outdb) != SQLITE_OK) {
		fprintf(stderr, "%s: %s: %s\n", argv[0], dbname, sqlite3_errmsg(outdb));
		__android_log_write(ANDROID_LOG_ERROR, "mbtiles_open", "not ok");
//		check_inside_open();
		exit(EXIT_FAILURE);
	}
//	__android_log_write(ANDROID_LOG_ERROR, "mbtiles_open", "OK");
//	check_after_open();
