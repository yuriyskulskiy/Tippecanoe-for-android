...
void checkdisk(struct reader *r, int nreader) {
	...
	if (used > diskfree * .9 && !warned) {
		...
		__android_log_write(ANDROID_LOG_INFO, "method checkdisk(): ",  "You will probably run out of disk space. %lld bytes used or committed, of %lld originally available");
		warned = 1;
	}
};
...
void init_cpus(const char *tmpdir) {
	...
	if (CPUS < 1) {
		CPUS = 1;
		__android_log_write(ANDROID_LOG_INFO, "...................1 ",  ".........................1");
	...
	if (MAX_FILES < 32) {
		...
		__android_log_write(ANDROID_LOG_INFO, "method init_cpus: ",  "Can't open a useful number of files:");
	}
	...
	while (head != NULL) {
		...
		*progress += (ix->end - ix->start) * 3 / 4;
		if (!quiet && 100 * *progress / *progress_max != *progress_reported) {
			...
			__android_log_write(ANDROID_LOG_INFO, "method merge(): ",  "Reordering geometry: %lld%%");
...
void *run_read_parallel(void *v) {
	...
	if (rpa->len != st.st_size) {
		...
		__android_log_write(ANDROID_LOG_INFO, "method *run_read_parallel(): ",  "wrong number of bytes in temporary: %lld vs %lld");
	}
	rpa->len = st.st_size;
...
//		__android_log_write(ANDROID_LOG_INFO, "method radix1(): ",  geomname);

		char indexname[strlen(tmpdir) + strlen("/index.XXXXXXXX") + 1];
		sprintf(indexname, "%s%s", tmpdir, "/index.XXXXXXXX");
//		__android_log_write(ANDROID_LOG_INFO, "method radix1(): ",  indexname);

		geomfds[i] = mkstemp(geomname);
		if (geomfds[i] < 0) {
			__android_log_write(ANDROID_LOG_INFO, "method radix1(): (geomfds[i] < 0)",  geomname);
		...
		geomfiles[i] = fopen(geomname, "wr");
		if (geomfiles[i] == NULL) {
			perror(geomname);
			__android_log_write(ANDROID_LOG_INFO, "method radix1(): wr",  geomname);

		}
		indexfiles[i] = fopen(indexname, "wr");
		if (indexfiles[i] == NULL) {
			perror(indexname);
			__android_log_write(ANDROID_LOG_INFO, "method radix1(): wr",  indexname);
	...
	for (i = 0; i < inputs; i++) {
		...
		if (fstat(geomfds_in[i], &geomst) < 0) {
			__android_log_write(ANDROID_LOG_INFO, "method radix1(): 538 ",  "perror(\"stat geom\");");

			perror("stat geom");
		}
		if (fstat(indexfds_in[i], &indexst) < 0) {
			perror("stat index");
			__android_log_write(ANDROID_LOG_INFO, "method radix1(): 544 ",  "perror(\"stat index\");");
		...
		if (indexst.st_size != 0) {
			...
			if (indexmap == MAP_FAILED) {
				...
				perror("map index");
				__android_log_write(ANDROID_LOG_INFO, "method radix1(): case: (indexmap == MAP_FAILED)  ",  "perror(\"map index\");");
			}
			...
			if (geommap == MAP_FAILED) {
				__android_log_write(ANDROID_LOG_INFO, "method radix1(): case: (geommap == MAP_FAILED)  ",  "perror(\"map geom\");");
				perror("map geom");
			}
			...
			for (size_t a = 0; a < indexst.st_size / sizeof(struct index); a++) {
				...
				*progress += (ix.end - ix.start) / 4;
				if (!quiet && 100 * *progress / *progress_max != *progress_reported) {
					fprintf(stderr, "Reordering geometry: %lld%% \r", 100 * *progress / *progress_max);
					__android_log_write(ANDROID_LOG_INFO, "method radix1(): case: if (!quiet && 100 * *progress / *progress_max != *progress_reported)  ",  "Reordering geometry:");
					*progress_reported = 100 * *progress / *progress_max;
				}
			...
			if (munmap(indexmap, indexst.st_size) < 0) {
				perror("unmap index");
				__android_log_write(ANDROID_LOG_INFO, "method radix1(): case: if (munmap(indexmap, indexst.st_size) < 0)  ",  "perror(\"unmap index\");");

			}
			madvise(geommap, geomst.st_size, MADV_DONTNEED);
			if (munmap(geommap, geomst.st_size) < 0) {
				perror("unmap geom");
				__android_log_write(ANDROID_LOG_INFO, "method radix1(): case: if (munmap(geommap, geomst.st_size) < 0)  ",  "perror(\"unmap geom\");");
		...
		if (close(geomfds_in[i]) < 0) {
			perror("close geom");
			__android_log_write(ANDROID_LOG_INFO, "method radix1(): case: if (close(geomfds_in[i]) < 0)  ",  "perror(\"close geom\");");
		}
		if (close(indexfds_in[i]) < 0) {
			perror("close index");
			__android_log_write(ANDROID_LOG_INFO, "method radix1(): case: if (close(indexfds_in[i]) < 0)  ",  "perror(\"close index\");");
		}
	...
	for (i = 0; i < splits; i++) {
		if (fclose(geomfiles[i]) != 0) {
			perror("fclose geom");
			__android_log_write(ANDROID_LOG_INFO, "method radix1(): case:  if (fclose(geomfiles[i]) != 0)  ",  "perror(\"fclose geom\");");
		}
		if (fclose(indexfiles[i]) != 0) {
			perror("fclose index");
			__android_log_write(ANDROID_LOG_INFO, "method radix1(): case:  if (fclose(indexfiles[i]) != 0) ",  "perror(\"fclose index\");");
	...
	for (i = 0; i < splits; i++) {
		...
		if (fstat(geomfds[i], &geomst) < 0) {
			perror("stat geom");
			__android_log_write(ANDROID_LOG_INFO, "method radix1(): case:  if (fstat(geomfds[i], &geomst) < 0) ",  "perror(\"stat geom\");");

		}
		if (fstat(indexfds[i], &indexst) < 0) {
			perror("stat index");
			__android_log_write(ANDROID_LOG_INFO, "method radix1(): case: if (fstat(indexfds[i], &indexst) < 0) ",  "perror(\"stat index\");");
		}
			...
			if (indexst.st_size + geomst.st_size < mem) {
				...
				for (a = 0; a < CPUS; a++) {
					...
					if (pthread_create(&pthreads[a], NULL, run_sort, &args[a]) != 0) {
						perror("pthread_create");
						__android_log_write(ANDROID_LOG_INFO, "method radix1(): case: if (pthread_create(&pthreads[a], NULL, run_sort, &args[a]) != 0)",  "perror(\"pthread_create\");");
				...
				for (a = 0; a < CPUS; a++) {
					...
					if (pthread_join(pthreads[a], &retval) != 0) {
						...
						__android_log_write(ANDROID_LOG_INFO, "method radix1(): case: if (pthread_join(pthreads[a], &retval) != 0) ",  "perror(\"pthread_join 679\");");
					}
				}
				...
				if (indexmap == MAP_FAILED) {
					...
					perror("map index");
					__android_log_write(ANDROID_LOG_INFO, "method radix1(): case: if (indexmap == MAP_FAILED) 685",  "perror(\"map index\");");
				}
				...
				if (geommap == MAP_FAILED) {
					perror("map geom");
					__android_log_write(ANDROID_LOG_INFO, "method radix1(): case: if (geommap == MAP_FAILED) 692",  "perror(\"map geom\");");
				}
				...
				if (munmap(indexmap, indexst.st_size) < 0) {
					perror("unmap index");
					__android_log_write(ANDROID_LOG_INFO, "method radix1(): case: if (munmap(indexmap, indexst.st_size) < 0) 702",  "perror(\"unmap index\");");
				}
				madvise(geommap, geomst.st_size, MADV_DONTNEED);
				if (munmap(geommap, geomst.st_size) < 0) {
					perror("unmap geom");
					__android_log_write(ANDROID_LOG_INFO, "method radix1(): case: if if (munmap(geommap, geomst.st_size) < 0) 707",  "perror(\"unmap geom\");");
				}
				...
				if (indexmap == MAP_FAILED) {
					fprintf(stderr, "fd %lld, len %lld\n", (long long) indexfds[i], (long long) indexst.st_size);
					perror("map index");
					__android_log_write(ANDROID_LOG_INFO, "method radix1(): case: if (indexst.st_size == sizeof(struct index) || prefix + splitbits >= 64)+if (indexmap == MAP_FAILED) 714",  "perror(\"map index\");");
				}
				madvise(indexmap, indexst.st_size, MADV_SEQUENTIAL);
				madvise(indexmap, indexst.st_size, MADV_WILLNEED);
				char *geommap = (char *) mmap(NULL, geomst.st_size, PROT_READ, MAP_PRIVATE, geomfds[i], 0);
				if (geommap == MAP_FAILED) {
					perror("map geom");
					__android_log_write(ANDROID_LOG_INFO, "method radix1(): case: if (indexst.st_size == sizeof(struct index) || prefix + splitbits >= 64)+if (geommap == MAP_FAILED) 721",  "perror(\"map geom\");");
				...
				for (size_t a = 0; a < indexst.st_size / sizeof(struct index); a++) {
					...
					*progress += (ix.end - ix.start) * 3 / 4;
					if (!quiet && 100 * *progress / *progress_max != *progress_reported) {
						fprintf(stderr, "Reordering geometry: %lld%% \r", 100 * *progress / *progress_max);
						__android_log_write(ANDROID_LOG_INFO, "method radix1(): case: if (!quiet && 100 * *progress / *progress_max != *progress_reported) 738 ",  " Reordering geometry: %lld%% ");
				...
				if (munmap(indexmap, indexst.st_size) < 0) {
					perror("unmap index");
					__android_log_write(ANDROID_LOG_INFO, "method radix1(): case: if (munmap(indexmap, indexst.st_size) < 0) 751 ",  " perror(\"unmap index\"); ");

				}
				madvise(geommap, geomst.st_size, MADV_DONTNEED);
				if (munmap(geommap, geomst.st_size) < 0) {
					perror("unmap geom");
					__android_log_write(ANDROID_LOG_INFO, "method radix1(): case: if (munmap(geommap, geomst.st_size) < 0) 757 ",  " perror(\"unmap geom\"); ");
				}
			} else {
		...
		if (!already_closed) {
			if (close(geomfds[i]) < 0) {
				perror("close geom");
				__android_log_write(ANDROID_LOG_INFO, "method radix1(): case:if (!already_closed)+ if (close(geomfds[i]) < 0) 775 ",  " perror(\"close geom\"); ");

			}
			if (close(indexfds[i]) < 0) {
				perror("close index");
				__android_log_write(ANDROID_LOG_INFO, "method radix1(): case:if (!already_closed)+  if (close(indexfds[i]) < 0) 780 ",  "perror(\"close index\"); ");
			}
...
void radix(struct reader *reader, int nreaders, FILE *geomfile, int geomfd, FILE *indexfile, int indexfd, const char *tmpdir, long long *geompos) {
	...
	if (pages < 0 || pagesize < 0) {
		...
		__android_log_write(ANDROID_LOG_INFO, "method radix(): case:if (pages < 0 || pagesize < 0)  813 ",  "perror(\"sysconf _SC_PAGESIZE or _SC_PHYS_PAGES\"); ");
	...
	for (i = 0; i < nreaders; i++) {
		...
		if (fstat(reader[i].geomfd, &geomst) < 0) {
			__android_log_write(ANDROID_LOG_INFO, "method radix(): case:if (fstat(reader[i].geomfd, &geomst) < 0)  848 ",  "perror(\"stat geom\"); ");
	...
	if (availfiles - 2 * nreaders != availfiles_before) {
		...
		__android_log_write(ANDROID_LOG_INFO, "method radix()end: case:if if (availfiles - 2 * nreaders != availfiles_before)  861 ",  "Internal error: miscounted available file descriptors: %lld vs %lld");
	...
	for (size_t i = 0; i < maps.size(); i++) {
		for (auto map = maps[i].begin(); map != maps[i].end(); ++map) {
			...
			if (out_entry == out.end()) {
				...
				__android_log_write(ANDROID_LOG_INFO, "method std::map<std::string, layermap_entry> merge_layermaps(): if (out_entry == out.end())  878 ",  "Internal error merging layers");
...
int read_input(std::vector<source> &sources, char *fname, const char *layername, int maxzoom, int minzoom, int basezoom, double basezoom_marker_width, sqlite3 *outdb, std::set<std::string> *exclude, std::set<std::string> *include, int exclude_all, double droprate, int buffer, const char *tmpdir, double gamma, int read_parallel, int forcetable, const char *attribution) {
	...
	int i;
    fprintf(stderr, "CPUS count %d\n",CPUS);
	__android_log_write(ANDROID_LOG_INFO, "method read_input:   904 ",  "CPUS count %d");
	...
	for (i = 0; i < CPUS; i++) {
		...
		__android_log_write(ANDROID_LOG_INFO, "method read_input:  916 ",tmpdir);
// не получается залогмровать     tmpdir+"/meta.XXXXXXXX"  -  invalid operands of types 'const char*' and 'const char [15]' to binary 'operator+'
		...
		if (r->metafd < 0) {
			__android_log_write(ANDROID_LOG_INFO, "method read_input:   937 ",  metaname);
		...
		if (r->poolfd < 0) {
			__android_log_write(ANDROID_LOG_INFO, "method read_input:   944 ",  poolname);

			perror(poolname);
			
		}
		// here is something wrong - android doesnt let or donno why... cant create tmp directory
		r->treefd = mkstemp(treename);
		if (r->treefd < 0) {
			__android_log_write(ANDROID_LOG_INFO, "method read_input:case if (r->treefd < 0)  936 ","perror(treename);");

			__android_log_write(ANDROID_LOG_INFO, "tag here------------------------------", "r->treefd = mkstemp(treename) => perror(treename)");
			perror(treename);
		...
		if (r->geomfd < 0) {
			perror(geomname);
			__android_log_write(ANDROID_LOG_INFO, "method read_input:case if (r->geomfd < 0)  945 ","perror(geomname);");
		...
		if (r->indexfd < 0) {
			perror(indexname);
			__android_log_write(ANDROID_LOG_INFO, "method read_input:case if  (r->geomfd < 0)  952 ","perror(indexname);");


		}

		r->metafile = fopen(metaname, "wr");
		if (r->metafile == NULL) {
			perror(metaname);
			__android_log_write(ANDROID_LOG_INFO, "method read_input:case if (r->metafile == NULL)  960 ","perror(metaname);");
		...
		if (r->poolfile == NULL) {
			perror(poolname);
			__android_log_write(ANDROID_LOG_INFO, "method read_input:case if if (r->poolfile == NULL) {  967 ","perror(poolname);");


		}
		r->treefile = memfile_open(r->treefd);
		if (r->treefile == NULL) {
			__android_log_write(ANDROID_LOG_INFO, "method read_input:case if (r->treefile == NULL)   973 ", "perror(treename)");
			__android_log_write(ANDROID_LOG_INFO, "method read_input:case if (r->treefile == NULL)   973 ", treename);
			perror(treename);
			
		}
		r->geomfile = fopen(geomname, "wr");
		if (r->geomfile == NULL) {
			__android_log_write(ANDROID_LOG_INFO, "method read_input:case if (r->geomfile == NULL)    980 ","perror(geomname);");
			__android_log_write(ANDROID_LOG_INFO, "method read_input:case if (r->geomfile == NULL)    980 ",geomname);
			perror(geomname);
			
		}
		r->indexfile = fopen(indexname, "wr");
		if (r->indexfile == NULL) {
			perror(indexname);
			__android_log_write(ANDROID_LOG_INFO, "method read_input:case if (r->indexfile == NULL)    988 ","perror(geomname);");
			__android_log_write(ANDROID_LOG_INFO, "method read_input:case if (r->indexfile == NULL)    889 ",indexname);
	...
	if (fstatfs(reader[0].geomfd, &fsstat) != 0) {
		...
		__android_log_write(ANDROID_LOG_INFO, "method read_input:case if (fstatfs(reader[0].geomfd, &fsstat) != 0)    1017 ","perror(\"fstatfs\");");
		perror("fstatfs");
	...
	for (size_t l = 0; l < nlayers; l++) {
		if (layername != NULL) {
			...
			if (!quiet) {
				...
				__android_log_write(ANDROID_LOG_INFO, "method read_input:case if  (!quiet)     1086 ","For layer %d, using name ");
				__android_log_write(ANDROID_LOG_INFO, "method read_input:case if  (!quiet)     1087 ",out.c_str());
	...
	for (size_t source = 0; source < nsources; source++) {
		...
		if (map != NULL && map != MAP_FAILED) {
			...
			if (munmap(map, st.st_size - off) != 0) {
				...
				perror("munmap source file");
				__android_log_write(ANDROID_LOG_INFO, "method read_input:case if (map != NULL && map != MAP_FAILED)+if (munmap(map, st.st_size - off) != 0)     1150 ", "perror(\"munmap source file\"); ");
			...
			if (fp == NULL) {
				...
				if (close(fd) != 0) {
					perror("close source file");
					__android_log_write(ANDROID_LOG_INFO, "method read_input:case if (fp == NULL)+if (close(fd) != 0)      1159 ", "perror(\"close source file\");");
			...
			if (read_parallel) {
				...
				__android_log_write(ANDROID_LOG_INFO, "method read_input:case if (read_parallel)      1171 ", "perror(\"close source file\");");
				__android_log_write(ANDROID_LOG_INFO, "method read_input:case if (read_parallel)      1171 ", readname);
				int readfd = mkstemp(readname);
				if (readfd < 0) {
					perror(readname);
					__android_log_write(ANDROID_LOG_INFO, "method read_input:case if (read_parallel) +if (readfd < 0)     1176 ", "perror(readname);");
					__android_log_write(ANDROID_LOG_INFO, "method read_input:case if (read_parallel) +if (readfd < 0)     1176 ", readname);

				}
				FILE *readfp = fdopen(readfd, "w");
				if (readfp == NULL) {
					perror(readname);
					__android_log_write(ANDROID_LOG_INFO, "method read_input:case if (read_parallel) +if (readfp == NULL)     1183 ", "perror(readname);");
					__android_log_write(ANDROID_LOG_INFO, "method read_input:case if (read_parallel) +if (readfp == NULL)     1183 ", readname);
...
#define PARSE_MAX (1LL * 1024 * 1024 * 1024)
						...
						if (is_parsing == 0 || ahead >= PARSE_MAX) {
							if (parser_created) {
								if (pthread_join(parallel_parser, NULL) != 0) {
									...
									__android_log_write(ANDROID_LOG_INFO, "method read_input:case if (pthread_join(parallel_parser, NULL) != 0)    1216 ", "perror(\"pthread_join 1088\");");
							...
							sprintf(readname, "%s%s", tmpdir, "/read.XXXXXXXX");
							__android_log_write(ANDROID_LOG_INFO, "method read_input:case if (is_parsing == 0 || ahead >= PARSE_MAX)    1232 ", "sprintf(readname, \"%s%s\", tmpdir, \"/read.XXXXXXXX\");");

							readfd = mkstemp(readname);
							if (readfd < 0) {
								perror(readname);
								__android_log_write(ANDROID_LOG_INFO, "method read_input:case if (readfd < 0)     1237 ", "perror(readname);");
								__android_log_write(ANDROID_LOG_INFO, "method read_input:case if (readfd < 0)     1237 ", readname);

							}
							readfp = fdopen(readfd, "w");
							if (readfp == NULL) {
								perror(readname);
								__android_log_write(ANDROID_LOG_INFO, "method read_input:case if  (readfp == NULL)    1244 ", "perror(readname);");
								__android_log_write(ANDROID_LOG_INFO, "method read_input:case if  (readfp == NULL)    1244 ", readname);
				...
				if (n < 0) {
					...
					__android_log_write(ANDROID_LOG_INFO, "method read_input:case if if (n < 0)   1255 ", "perror(reading.c_str())");
					__android_log_write(ANDROID_LOG_INFO, "method read_input:case if if (n < 0)   1255 ", reading.c_str());
				...
				if (parser_created) {
					if (pthread_join(parallel_parser, NULL) != 0) {
						...
						__android_log_write(ANDROID_LOG_INFO, "method read_input:case if (parser_created)+if (pthread_join(parallel_parser, NULL) != 0)    1263 ", "perror(\"pthread_join 1122\");");
				...
				if (ahead > 0) {
					...
					if (parser_created) {
						if (pthread_join(parallel_parser, NULL) != 0) {
							...
							__android_log_write(ANDROID_LOG_INFO, "method read_input:case if (parser_created)+if if (pthread_join(parallel_parser, NULL) != 0)    1277 ", "perror(\"pthread_join 1133\");");
			...
			if (fclose(fp) != 0) {
				perror("fclose input");
				__android_log_write(ANDROID_LOG_INFO, "method read_input:case if (fclose(fp) != 0)    1299 ", "perror(\"fclose input\");");
	...
	if (!quiet) {
		...
		__android_log_write(ANDROID_LOG_INFO, "method read_input:case if (fclose(fp) != 0)    1309 ", "(stderr, \"Read 10000.00 million features\\r\", *progress_seq / 1000000.0);");
	...
	for (i = 0; i < CPUS; i++) {
		if (fclose(reader[i].metafile) != 0) {
			perror("fclose meta");
			__android_log_write(ANDROID_LOG_INFO, "method read_input:case if (fclose(reader[i].metafile) != 0)   1316 ", "perror(\"fclose meta\");");


		}
		if (fclose(reader[i].geomfile) != 0) {
			perror("fclose geom");
			__android_log_write(ANDROID_LOG_INFO, "method read_input:case if (fclose(reader[i].geomfile) != 0)   1322 ", "perror(\"fclose geom\");");

		}
		if (fclose(reader[i].indexfile) != 0) {
			perror("fclose index");
			__android_log_write(ANDROID_LOG_INFO, "method read_input:case if (fclose(reader[i].indexfile) != 0)   1327 ", "perror(\"fclose index\");");
		...
		if (fstat(reader[i].geomfd, &reader[i].geomst) != 0) {
			perror("stat geom\n");
			__android_log_write(ANDROID_LOG_INFO, "method read_input:case if (fstat(reader[i].geomfd, &reader[i].geomst) != 0)   1334 ", "perror(\"stat geom\");");
		...
		if (fstat(reader[i].metafd, &reader[i].metast) != 0) {
			perror("stat meta\n");
			__android_log_write(ANDROID_LOG_INFO, "method read_input:case if if (fstat(reader[i].metafd, &reader[i].metast) != 0)   1340 ", "perror(\"stat meta\");");
	...
	FILE *poolfile = fopen(poolname, "wr");
	if (poolfile == NULL) {
		perror(poolname);
	...
	FILE *metafile = fopen(metaname, "wr");
	if (metafile == NULL) {
		perror(metaname);
	...
	FILE *indexfile = fopen(indexname, "wr");
	if (indexfile == NULL) {
		perror(indexname);
	...
	FILE *geomfile = fopen(geomname, "wr");
	if (geomfile == NULL) {
		perror(geomname);
...
void check_after_parsing() {
	__android_log_assert("cond", "runMain", "after parsing");
}

int runMain(int argc, char **argv) {
//	__android_log_assert("cond", "runMain", "at the beginning");
#ifdef MTRACE
    mtrace();
#endif
...
//    open_file_limit.rlim_cur = 999999;
...
//	__android_log_assert("cond", "runMain", "before parsing");
...
//	__android_log_assert("cond", "runMain", "before mbtiles_open");
    sqlite3 *outdb = mbtiles_open(outdir, argv, forcetable);
//	__android_log_assert("cond", "runMain", "after mbtiles_open");
    int ret = EXIT_SUCCESS;
...
//	__android_log_assert("cond", "runMain", "after mbtiles_close");
#ifdef MTRACE
    muntrace();
#endif
...
int render_vector_tiles(bool rewrite, bool rg, char *outdir, char *tmpdir, int layar_count, char **layars, int argc, char **argv) {
    ...
    for (int i = 0; i < layar_count; i++) {
        ...
        struct source src;
        src.layer = std::string(val).substr(0, cp - val);
        src.file = std::string(cp + 1);
        sources.push_back(src);
    }


//	__android_log_assert("cond", "runMain", "here");
